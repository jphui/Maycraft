...
{ private Cat cat;
  ...
  private Coin coin;
    ...
    public Area3()
    {
        ...
        randomlyGenerateArea(3);
        setBackground("img/end.jpg");//background image
        render();
        cat = new Cat();
    ...
    public void update()
    {
       ...
       spawnMemeMan(); //spawn the herobrines
          ...
          if(cat.returnscore()>50000)getMayflower().setStage(new YouWinWorld()); //if the score is enough set the game to level 2
    }
    public void spawnMemeMan() //spawns Meme Man randomly, constantly
    {
        int edwin = (int)(Math.random()*75); //1 out of 150 chance to spawn during each iteration of the game loop
